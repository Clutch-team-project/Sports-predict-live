(function () {
    ...
    window.authFetch = async function (url, options) {
        ...
        if (res.status === 401) {
            ...
            var reissueRes = await fetch('/api/auth/reissue', {
                method: 'POST',
    ...
    function injectStyles() {
        ...
        s.textContent = [
            ...
            '.fl-shell{width:min(80rem,calc(100% - 3rem));margin:0 auto;padding:1rem 0 0;position:relative;z-index:10000;}',
            '.fl-bar{height:3.625rem;background:#fff;border:1px solid #cbd5e1;border-radius:1rem;box-shadow:0 4px 12px rgba(15,23,42,.14);display:flex;align-items:center;justify-content:space-between;padding:0 1.25rem;font-family:"Noto Sans KR",-apple-system,sans-serif;}',
            '.fl-logo{color:#2563eb;font-size:1.25rem;font-weight:900;letter-spacing:-.03em;cursor:pointer;}',
            '.fl-sports{display:flex;align-items:center;gap:2.125rem;font-size:1.0625rem;font-weight:900;color:#64748b;}',
            '.fl-sport{position:relative;padding:1.125rem 0;cursor:pointer;transition:color .15s;}',
            '.fl-sport:hover{color:#1e293b;}',
            '.fl-sport.is-active{color:#2563eb;}',
            '.fl-sport.is-active::after{content:"";position:absolute;left:0;right:0;bottom:.75rem;height:2px;background:#3b82f6;border-radius:10px;}',
            '.fl-menu{position:absolute;left:50%;top:100%;transform:translateX(-50%) translateY(.5rem);min-width:18.75rem;background:#fff;border:1px solid #cbd5e1;border-radius:1rem;box-shadow:0 12px 32px rgba(15,23,42,.18);padding:.625rem;display:flex;align-items:center;justify-content:center;gap:.25rem;opacity:0;pointer-events:none;transition:opacity .14s,transform .14s;}',
            '.fl-sport:hover .fl-menu{opacity:1;pointer-events:auto;transform:translateX(-50%) translateY(0);}',
            '.fl-menu button{border:0;background:transparent;color:#64748b;font-size:.875rem;font-weight:900;padding:.5625rem 1rem;border-radius:.625rem;cursor:pointer;font-family:inherit;white-space:nowrap;}',
            '.fl-menu button:hover{background:#eff6ff;color:#2563eb;}',
            '.fl-actions{display:flex;align-items:center;gap:.875rem;font-size:.875rem;font-weight:900;color:#334155;}',
            ...
            '.fl-mypage{display:inline-flex;align-items:center;gap:.3125rem;border:1px solid #cbd5e1;border-radius:.625rem;padding:.4375rem .75rem;background:#fff;color:#334155;transition:all .15s;}',
            '.fl-mypage::before{content:"\\1F464";font-size:.75rem;}',
